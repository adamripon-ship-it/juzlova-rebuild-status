...
# WordPress-era root URLs that Search Console still reports as 404 and that have
# a real equivalent today. Root only (they never existed under /en/ /de/ /sk/),
# so nginx and Netlify rules only, no meta-refresh stubs.
WP_REDIRECTS = {
    "chlupate_knedliky": "chlupate_knedliky",
    "kakaovy_prasek": "kakao_holandskeho_typu",
    "bramborove_knedliky-2": "bramborove_knedliky",
    "home-2": "home", "home-2-2": "home", "juzlova-potravinarske-smesi": "home",
    "sitemap": "home", "amp": "home",
    "Jiřina Jůzlová": "kontakt", "Jiří Jůzl": "kontakt",
}
# Gone for good (410): WordPress machinery, theme demo posts, feeds without a
# page, the old recovery archive and status board. robots.txt must not block
# these, or Google never sees the 410 and keeps them as "Blocked by robots.txt".
GONE_PREFIXES = ["wp-content", "wp-includes", "wp-json", "wp-admin", "author",
                 "category", "tag", "comments", "archive"]
GONE_PATHS = ["feed", "xmlrpc.php", "wp-login.php", "status.html", "__forms.html",
              "hello-world", "sample-page", "adam-kennedy-ripon",
              "class-aptent-taciti-sociosqu-ad-litora",
              "donec-at-mauris-enim-duis-nisi-tellus", "nullam-vitae-nibh-un-odio",
              "nunc-tincidunt-elit-non-cursus", "praesent-et-urna-turpis-sadips",
              "proin-sodales-quam-nec-ante-sollicits"]
...
def b2b_showcase_html(L, depth=0):
    ...
    # The per-place wholesale pages are Czech only; in en/de/sk these links
    # would 301 straight back to this page (Search Console: "Page with redirect").
    chips = ""
    if L["code"] == "cs":
        links = "".join(
...
<div class="b2b-tiles">{cards}</div>{chips}
</div>"""
...
def build_redirects(langs_data):
    ...
    gone = ("^/(?:" + "|".join(re.escape(p) for p in GONE_PREFIXES) + ")(?:/|$)|"
            "^/(?:" + "|".join(re.escape(p) for p in GONE_PATHS) + ")/?$")
    nginx = ["# Generated by scripts/build_site.py — old URLs -> new localized URLs (301).",
             "# Gone for good: 410 so Google drops them from the index.",
             f'if ($uri ~ "{gone}") {{ return 410; }}']
...
""")
        nginx.append(f"rewrite ^{re.escape(old_abs.rstrip('/'))}(?:/feed)?/?$ {target} permanent;")
        netlify.append(f"{old_abs.rstrip('/')}  {target}  301!")
        netlify.append(f"{old_abs}  {target}  301!")
    for old, pid in sorted(WP_REDIRECTS.items()):
        target = url_of("cs", pid)
        rx = "^/" + re.escape(old).replace("\\ ", " ") + "(?:/feed)?/?$"
        nginx.append(f'rewrite "{rx}" {target} permanent;')
        quoted = "/" + urllib.parse.quote(old)
        netlify.append(f"{quoted}  {target}  301!")
        netlify.append(f"{quoted}/  {target}  301!")
    # The WordPress feed of any page that still exists -> that page.
    nginx.append(f"rewrite ^(/.+)/feed/?$ {BASE}$1/ permanent;")
    netlify.append(f"/*/feed  {BASE}/:splat/  301!")
    netlify.append(f"/*/feed/  {BASE}/:splat/  301!")
...
# No Disallow: the old /archive/ and /status.html answer 410 Gone, and a
# robots.txt block would hide that from Google.
User-agent: *
Allow: /
